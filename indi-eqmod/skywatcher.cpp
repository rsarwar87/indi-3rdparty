...
void Skywatcher::InquireEncoderInfo(SkywatcherAxis axis, double *steppersvalues)
{
...
#ifdef _KOHERON
    *Steps360 = koheron_interface->SwpGetGridPerRevolution(Axis2);
    if (*Steps360 == 0xFFFFFFFF)
    {
    ...
    *HighspeedRatio = (uint32_t)koheron_interface->SwpGetHighSpeedRatio(Axis2);
...
#ifdef _KOHERON
    steppersvalues[1] = static_cast<double>(*StepsWorm)/5;
#else
    steppersvalues[1] = static_cast<double>(*StepsWorm)/5;
#endif
